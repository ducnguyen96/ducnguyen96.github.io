...
We could rewrite our sample server as follows, without using the default mux. Only the `main` function changes, so I won't show the `hello` and `headers` handlers, but you can see the full code [here](https://github.com/ducnguyen96/ducnguyen96.github.io/tree/master/static/code/blog/go-life-http-request/basic-server-mux-object.go). There's no change in functionality [^1]:
